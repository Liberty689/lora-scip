...
def gradio_config():
    with gr.Accordion('Configuration file (配置文件)', open=False):
        with gr.Row():
            button_open_config = gr.Button('Open 📂（打开）', elem_id='open_folder')
            button_save_config = gr.Button('Save 💾（保存）', elem_id='open_folder')
            button_save_as_config = gr.Button(
                'Save as... 💾（另存为）', elem_id='open_folder'
            )
...
def gradio_training(
    ...
    with gr.Row():
        ...
        cache_latents = gr.Checkbox(label='Cache latent（缓存潜在编码）', value=True)
    with gr.Row():
        ...
        lr_warmup = gr.Textbox(
            label='LR warmup (% of steps) (学习率预热步数)', value=lr_warmup_value
        )
...
def gradio_advanced_training():
    ...
    with gr.Row():
        ...
        min_snr_gamma = gr.Slider(label='Min SNR gamma (最小SNR gamma)', value = 0, minimum=0, maximum=20, step=1)
    with gr.Row():
        ...
        noise_offset = gr.Textbox(
            label='Noise offset (0 - 1)（噪音偏移）', placeholder='(Oprional 可选) eg: 0.1'
        )
    ...
    with gr.Row():
        ...
        max_data_loader_n_workers = gr.Textbox(
            ...
            placeholder='(Optional 可选) Override number of epoch. Default: 8',
            value="0",
        )
    return (
        # use_8bit_adam,

    ...
    with gr.Tab('Training parameters（训练参数）'):
        with gr.Row():
            ...
            lora_network_weights = gr.Textbox(
                ...
                placeholder='{Optional 可选) Path to existing LoRA network weights to resume training',
            )
        ...
        with gr.Row():
            ...
            enable_bucket = gr.Checkbox(label='Enable buckets （启用桶）', value=True,
        ...
        with gr.Accordion('Advanced Configuration (高级设置)', open=False):
                ...
                with gr.Tab(label='Conv'):
                    with gr.Row(visible=True):
                        conv_dims = gr.Textbox(
                            ...
                            placeholder='(Optional 可选) eg: 2,2,2,2,4,4,4,4,6,6,6,6,8,6,6,6,6,4,4,4,4,2,2,2,2',
            ...
            with gr.Row():
                ...
                weighted_captions = gr.Checkbox(
                    label='Weighted captions (带有权重的标注)', value=False, info='Enable weighted captions in the standard style (token:1.3). No commas inside parens, or shuffle/dropout may break the decoder.',
                )
            with gr.Row():
                ...
                lr_scheduler_num_cycles = gr.Textbox(
                    ...
                    placeholder='(Optional 可选) For Cosine with restart and polynomial only',
                )
